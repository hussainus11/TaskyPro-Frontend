...
interface EcommerceRecentOrdersCardProps {
  orders?: any[];
  loading?: boolean;
}

export function EcommerceRecentOrdersCard({ orders: propOrders = [], loading = false }: EcommerceRecentOrdersCardProps) {
  // Transform orders to match the component's Order type
  const orders: Order[] = React.useMemo(() => {
    return propOrders.slice(0, 20).map((order: any) => {
      const firstItem = order.items?.[0];
      const product = firstItem?.product;
      
      // Map database status to UI status
      let uiStatus: "processing" | "paid" | "success" | "failed" = "processing";
      ...
      return {
        ...
        customer: {
          ...
          image: order.customer?.image || `/images/avatars/01.png`
        },
        product: {
