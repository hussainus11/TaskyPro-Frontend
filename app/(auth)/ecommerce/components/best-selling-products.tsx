...
interface EcommerceBestSellingProductsCardProps {
  products?: any[];
  orders?: any[];
  loading?: boolean;
}

export function EcommerceBestSellingProductsCard({ products = [], orders = [], loading = false }: EcommerceBestSellingProductsCardProps) {
  // Calculate product sales from orders
  const productSales = React.useMemo(() => {
    const salesMap: Record<number, { product: any; sold: number; sales: number }> = {};

    // Process all orders to calculate sales
      ...
      if (order.items && Array.isArray(order.items)) {
        order.items.forEach((item: any) => {
          ...
          if (productId) {
            ...
            salesMap[productId].sold += price * quantity;
            salesMap[productId].sales += quantity;
          }
        });
      }
    });
    ...
    return Object.values(salesMap)
      .map((item) => ({
        ...
        image: item.product?.image || `/images/products/01.jpeg`,
