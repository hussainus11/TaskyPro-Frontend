...
export function FieldPermissionsDialog({
  ...
  useEffect(() => {
    if (open && fieldId && !isNaN(fieldId)) {
      console.log('Loading permissions for field:', fieldId);
      loadData();
    } else {
      console.log('Dialog open but invalid fieldId:', { open, fieldId, isNaN: isNaN(fieldId) });
      setPermissions([]);
      setPermissionMap({});
      setSearchQuery("");
    }
  ...
  const loadData = async () => {
    try {
      ...
      const existingPermissions = await formPermissionsApi.getFieldPermissions(fieldId);
  ...
  const handleSave = async () => {
    try {
      ...
      await formPermissionsApi.bulkUpdateFieldPermissions(fieldId, permissionsToUpdate);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[80vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Control who can view the field: <strong>{fieldLabel}</strong>
          </DialogDescription>
        </DialogHeader>
