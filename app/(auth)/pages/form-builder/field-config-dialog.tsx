...
import { Plus, Trash2, GripVertical, Settings, ShieldCheck, SlidersHorizontal, Lock, Eye } from "lucide-react";
import {
  arrayMove,
  SortableContext,
...
const fieldConfigSchema = z.object({
  ...
  encryptInDb: z.boolean().optional(),
  visibleChars: z.number().int().min(0).optional()
...
export function FieldConfigDialog({
  ...
  const form = useForm<FieldConfigFormValues>({
    ...
    defaultValues: {
      ...
      encryptInDb: (field as any).encryptInDb || false,
      visibleChars: (field as any).visibleChars ?? undefined
    }
  });
  ...
  const onSubmit = async (data: FieldConfigFormValues) => {
    ...
    try {
      const updatedField: FormFieldType = {
        ...
        encryptInDb: data.encryptInDb || undefined,
        visibleChars: data.visibleChars !== undefined && data.visibleChars > 0 ? data.visibleChars : undefined,
      } as FormFieldType;
              ...
              <TabsContent value="basic" className="flex-1 overflow-y-auto">
                <div className="space-y-4">
                  ...
                  <div className="border-t pt-4 space-y-4">
                    <div>
                      <h3 className="text-sm font-semibold">Data Security</h3>
                      <p className="text-xs text-muted-foreground mt-0.5">
                        Control how this field's value is stored and displayed
                      </p>
                    </div>
                      ...
                      render={({ field }) => (
                        <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                          <div className="space-y-0.5">
                            ...
                            <FormDescription>
                              Value will be encrypted before being stored in the database
                            </FormDescription>
                          </div>
                          <FormControl>
                            <Switch
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="flex items-center gap-2">
                            ...
                            Visible Characters in UI
                          </FormLabel>
                          <FormControl>
                            <Input
                              type="number"
                              min={0}
                              placeholder="e.g. 4 — shows last 4 chars, rest masked"
                          ...
                          <FormDescription>
                            Number of characters to reveal when displaying this value (e.g. 4 → ••••••1234). Leave empty to show the full value.
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
