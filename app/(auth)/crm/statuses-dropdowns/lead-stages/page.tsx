...
export default function LeadStagesPage() {
  ...
  useEffect(() => {
    const loadStages = async () => {
      try {
        setLoading(true);
        const data = await leadStagesApi.getLeadStages();
        ...
        if (initial) {
          ...
          try {
            const newStage = await leadStagesApi.createLeadStage({
              name: "Initial Stage",
              color: colorMap["bg-blue-500"],
              order: 1,
              type: 'initial'
        ...
        // Success is always exactly one fixed stage, auto-created if missing
        if (success.length > 0) {
          ...
          try {
            ...
            console.error('Failed to create success stage:', createError);
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    // Check if dropped into failed droppable area
    if (overId === 'failed-area') {
      const stageToMove = stages.find((s) => s.id === activeIdNum);
      if (stageToMove) {
        const newOrder = 10 + failedStagesList.length + 1;
        try {
          await leadStagesApi.updateLeadStage(activeIdNum, { type: 'failed', order: newOrder });
          const updatedStages = stages
            .filter((stage) => stage.id !== activeIdNum)
            .map((stage, index) => ({ ...stage, order: index + 2 }));
      ...
      if (oldIndex !== -1 && newIndex !== -1) {
        ...
        try {
          await leadStagesApi.reorderLeadStages(
  ...
  const handleNameChange = async (id: number, name: string) => {
    try {
      await leadStagesApi.updateLeadStage(id, { name });
      
      // Update the stage in the appropriate list
      if (initialStage && id === initialStage.id) {
        setInitialStage({ ...initialStage, name });
      } else if (successStage && id === successStage.id) {
        setSuccessStage({ ...successStage, name });
      } else {
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Are you sure you want to delete this stage?")) {
      try {
        await leadStagesApi.deleteLeadStage(id);
        const updatedStages = stages
  ...
  const handleColorChange = async (id: number, color: string) => {
    try {
      await leadStagesApi.updateLeadStage(id, { color });
      if (initialStage && id === initialStage.id) {
        setInitialStage({ ...initialStage, color });
      } else if (successStage && id === successStage.id) {
        setSuccessStage({ ...successStage, color });
      } else {
        setStages(stages.map((stage) =>
  ...
  const handleInitialStageColorChange = async (color: string) => {
    ...
    try {
      await leadStagesApi.updateLeadStage(initialStage.id, { color });
      setInitialStage({ ...initialStage, color });
    } catch (error: any) {
  ...
  const handleInitialStageNameChange = async (name: string) => {
    if (!initialStage) return;
    try {
      await leadStagesApi.updateLeadStage(initialStage.id, { name });
      setInitialStage({ ...initialStage, name });
      toast.success('Stage name updated');
    } catch (error: any) {
      console.error('Failed to update stage name:', error);
  ...
  const handleSuccessStageColorChange = async (color: string) => {
    ...
    try {
      await leadStagesApi.updateLeadStage(successStage.id, { color });
      setSuccessStage({ ...successStage, color });
    } catch (error: any) {
      console.error('Failed to update color:', error);
      toast.error('Failed to update stage color');
  ...
  const handleSuccessStageNameChange = async (name: string) => {
    if (!successStage) return;
    try {
      await leadStagesApi.updateLeadStage(successStage.id, { name });
      setSuccessStage({ ...successStage, name });
      toast.success('Stage name updated');
    } catch (error: any) {
      console.error('Failed to update stage name:', error);
  ...
  const handleSave = async () => {
    try {
      ...
      await leadStagesApi.reorderLeadStages(reorderData);
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            {/* Success Stage — single fixed stage like Initial */}
            <div className="space-y-2">
              ...
              <div className="border rounded-lg p-4 space-y-3">
                ...
                {successStage && (
                  <InitialStageCard
                    stage={successStage}
                    onColorChange={handleSuccessStageColorChange}
                    onEdit={handleEdit}
                    onNameChange={handleSuccessStageNameChange}
                  />
                )}
              </div>
            </div>
