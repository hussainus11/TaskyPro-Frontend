...
const defaultSuccessStage: Stage = {
  id: 0, name: "Signed", color: "#10b981", order: 10, type: 'success'
};
...
export default function DocumentStagesPage() {
    ...
    const loadStages = async () => {
      try {
        ...
        if (initial) {
          ...
          try {
            const newStage = await documentStagesApi.createDocumentStage({ name: "Initial Stage", color: colorMap["bg-blue-500"], order: 1, type: 'initial' });
        ...
        if (success.length > 0) {
          ...
          try {
            const newStage = await documentStagesApi.createDocumentStage({ name: "Signed", color: "#10b981", order: 10, type: 'success' });
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (overId === 'failed-area') {
      ...
      if (stageToMove) {
        ...
        try {
          await documentStagesApi.updateDocumentStage(activeIdNum, { type: 'failed', order: newOrder });
      ...
      if (oldIndex !== -1 && newIndex !== -1) {
        ...
        try {
          await documentStagesApi.reorderDocumentStages(updatedStages.map((s) => ({ id: s.id, order: s.order })));
        } catch {
  ...
  const handleInitialStageColorChange = async (color: string) => {
    ...
    try { await documentStagesApi.updateDocumentStage(initialStage.id, { color }); setInitialStage({ ...initialStage, color }); }
    catch { toast.error('Failed to update stage color'); }
  };

  const handleInitialStageNameChange = async (name: string) => {
    if (!initialStage) return;
    try { await documentStagesApi.updateDocumentStage(initialStage.id, { name }); setInitialStage({ ...initialStage, name }); toast.success('Stage name updated'); }
  ...
  const handleSuccessStageColorChange = async (color: string) => {
    ...
    try { await documentStagesApi.updateDocumentStage(successStage.id, { color }); setSuccessStage({ ...successStage, color }); }
    catch { toast.error('Failed to update stage color'); }
  };

  const handleSuccessStageNameChange = async (name: string) => {
    if (!successStage) return;
    try { await documentStagesApi.updateDocumentStage(successStage.id, { name }); setSuccessStage({ ...successStage, name }); toast.success('Stage name updated'); }
    catch { toast.error('Failed to update stage name'); }
  };

  const handleAddStage = async () => {
    try {
      const newStage = await documentStagesApi.createDocumentStage({ name: "New Stage", color: presetColors[0], order: stages.length + 2, type: 'additional' });
  ...
  const handleFailedColorChange = async (id: number, color: string) => {
    try { await documentStagesApi.updateDocumentStage(id, { color }); setFailedStagesList(failedStagesList.map((s) => s.id === id ? { ...s, color } : s)); }
    catch { toast.error('Failed to update stage color'); }
  };

  const handleAddFailedStage = async () => {
    try {
      const newStage = await documentStagesApi.createDocumentStage({ name: "New Failed Stage", color: "#60a5fa", order: 10 + failedStagesList.length + 1, type: 'failed' });
  ...
  const handleDeleteFailedStage = async (id: number) => {
    if (confirm("Are you sure you want to delete this stage?")) {
      try { await documentStagesApi.deleteDocumentStage(id); setFailedStagesList(failedStagesList.filter((s) => s.id !== id)); toast.success('Stage deleted successfully'); }
  ...
  const handleSave = async () => {
    try {
      ...
      await documentStagesApi.reorderDocumentStages(allStages.map((s) => ({ id: s.id, order: s.order })));
