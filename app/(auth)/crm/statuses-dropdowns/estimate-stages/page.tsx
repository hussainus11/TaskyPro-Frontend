...
import { estimateStagesApi } from "@/lib/api";
...
const defaultSuccessStage: Stage = {
  id: 0, name: "Accepted", color: "#10b981", order: 10, type: 'success'
};
...
export default function EstimateStagesPage() {
  ...
  useEffect(() => {
    const loadStages = async () => {
      try {
        setLoading(true);
        const data = await estimateStagesApi.getEstimateStages();
        ...
        if (initial) {
          ...
          try {
            const newStage = await estimateStagesApi.createEstimateStage({ name: "Initial Stage", color: colorMap["bg-blue-500"], order: 1, type: 'initial' });
        ...
        if (success.length > 0) {
          ...
          try {
            const newStage = await estimateStagesApi.createEstimateStage({ name: "Accepted", color: "#10b981", order: 10, type: 'success' });
        ...
        toast.error('Failed to load estimate stages');
        setInitialStage(defaultInitialStage);
      } finally {
        setLoading(false);
      }
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (overId === 'failed-area') {
      ...
      if (stageToMove) {
        ...
        try {
          await estimateStagesApi.updateEstimateStage(activeIdNum, { type: 'failed', order: newOrder });
      ...
      if (oldIndex !== -1 && newIndex !== -1) {
        ...
        try {
          await estimateStagesApi.reorderEstimateStages(updatedStages.map((s) => ({ id: s.id, order: s.order })));
        } catch {
  ...
  const handleNameChange = async (id: number, name: string) => {
    try {
      await estimateStagesApi.updateEstimateStage(id, { name });
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Are you sure you want to delete this stage?")) {
      try {
        await estimateStagesApi.deleteEstimateStage(id);
  ...
  const handleColorChange = async (id: number, color: string) => {
    try {
      await estimateStagesApi.updateEstimateStage(id, { color });
  ...
  const handleInitialStageColorChange = async (color: string) => {
    ...
    try { await estimateStagesApi.updateEstimateStage(initialStage.id, { color }); setInitialStage({ ...initialStage, color }); }
    catch { toast.error('Failed to update stage color'); }
  };

  const handleInitialStageNameChange = async (name: string) => {
    if (!initialStage) return;
    try { await estimateStagesApi.updateEstimateStage(initialStage.id, { name }); setInitialStage({ ...initialStage, name }); toast.success('Stage name updated'); }
  ...
  const handleSuccessStageColorChange = async (color: string) => {
    ...
    try { await estimateStagesApi.updateEstimateStage(successStage.id, { color }); setSuccessStage({ ...successStage, color }); }
    catch { toast.error('Failed to update stage color'); }
  };

  const handleSuccessStageNameChange = async (name: string) => {
    if (!successStage) return;
    try { await estimateStagesApi.updateEstimateStage(successStage.id, { name }); setSuccessStage({ ...successStage, name }); toast.success('Stage name updated'); }
    catch { toast.error('Failed to update stage name'); }
  };

  const handleAddStage = async () => {
    try {
      const newStage = await estimateStagesApi.createEstimateStage({ name: "New Stage", color: presetColors[0], order: stages.length + 2, type: 'additional' });
  ...
  const handleFailedColorChange = async (id: number, color: string) => {
    try { await estimateStagesApi.updateEstimateStage(id, { color }); setFailedStagesList(failedStagesList.map((s) => s.id === id ? { ...s, color } : s)); }
    catch { toast.error('Failed to update stage color'); }
  };

  const handleAddFailedStage = async () => {
    try {
      const newStage = await estimateStagesApi.createEstimateStage({ name: "New Failed Stage", color: "#60a5fa", order: 10 + failedStagesList.length + 1, type: 'failed' });
  ...
  const handleDeleteFailedStage = async (id: number) => {
    if (confirm("Are you sure you want to delete this stage?")) {
      try { await estimateStagesApi.deleteEstimateStage(id); setFailedStagesList(failedStagesList.filter((s) => s.id !== id)); toast.success('Stage deleted successfully'); }
  ...
  const handleSave = async () => {
    try {
      ...
      await estimateStagesApi.reorderEstimateStages(allStages.map((s) => ({ id: s.id, order: s.order })));
