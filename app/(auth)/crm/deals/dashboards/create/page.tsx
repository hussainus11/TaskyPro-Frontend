...
export default function CreateDealDashboardPage() {
  ...
  React.useEffect(() => {
    const loadFieldNames = async () => {
      try {
        // Prefer template-defined fields for DEAL (most stable)
        ...
        // Fallback: sample an entity record keys
  ...
  const toggleRole = (id: number) => {
    setSharedRoleIds((prev) => (prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]));
  };
  ...
  const compileBasicToAdvanced = () => {
    const parts: string[] = [];
    // Pipeline should not imply "all stages" when a specific stage is selected.
    // If a pipeline is selected and stage is "Any", then constrain to that pipeline's stages.
    if (pipelineId) {
      const pid = parseInt(pipelineId, 10);
      const hasSpecificStage = stageId && stageId !== "__any__";
      if (!Number.isNaN(pid) && !hasSpecificStage && stages.length > 0) {
  ...
  const tryConvertAdvancedToBasic = (q: string) => {
    ...
    const walk = (ast: DealQueryAst): boolean => {
      ...
      // OR / nested expressions are not safely convertible to Basic
      return false;
    };
    if (!walk(parsed.ast)) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await dealDashboardsApi.create({
        name: name.trim(),
        ...
        visibility,
        sharedRoleIds: visibility === "ROLES" ? sharedRoleIds : undefined,
        sharedUserIds: visibility === "USERS" ? sharedUserIds : undefined,
      });
      toast.success("Dashboard created");
      router.push(`/crm/deals/dashboards`);
    } catch (e: any) {
      console.error(e);
  ...
  // Avoid hydration mismatches from SSR + client-only user/menu data.
  // IMPORTANT: don't early-return before hooks, otherwise hook order changes between renders.
  if (!mounted) {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        <h1 className="text-2xl font-bold tracking-tight">Create Deal Dashboard</h1>
        <p className="text-muted-foreground text-sm">
          This saves a reusable filter + a dashboard layout/widgets definition. Opening a dashboard applies the filter on the Deals board.
        </p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label>Name</Label>
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. High value deals - mine" />
          </div>
          <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {roles.map((r) => (
                  <label key={r.id} className="flex items-center gap-2 text-sm">
                    <input
                      type="checkbox"
                      checked={sharedRoleIds.includes(r.id)}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>What the dashboard opens with on the Deals board.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Tabs
            defaultValue="basic"
            ...
            onValueChange={(v) => {
              ...
              if (v === "basic" && query.trim()) {
                ...
                // Switch to Basic as source of truth
                setQuery("");
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Stored immediately with the dashboard.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="flex items-center justify-between gap-3">
            <div>
              <div className="font-medium">Enable widget strip</div>
              <div className="text-sm text-muted-foreground">Saved into `widgets` JSON (rendering can evolve).</div>
            </div>
