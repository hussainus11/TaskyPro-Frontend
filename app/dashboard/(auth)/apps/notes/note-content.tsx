...
export default function NoteContent() {
  ...
  const fetchNotes = async () => {
    try {
      ...
      const fetchedNotes = await fetch(`${API_BASE_URL}/notes`);
      ...
      if (fetchedNotes.ok) {
        ...
        const mappedNotes = data.map((note: any) => ({
          ...
          image: note.image,
          labels: note.labels ? note.labels.map((label: any) => label.id) : [],
          items: note.checklistItems ? note.checklistItems.map((item: any) => ({
            text: item.text,
      ...
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        <div className="flex">
          ...
          <div className="block xl:hidden">
            <AddNoteModal onNoteCreated={fetchNotes} />
          </div>
        </div>
      </div>
