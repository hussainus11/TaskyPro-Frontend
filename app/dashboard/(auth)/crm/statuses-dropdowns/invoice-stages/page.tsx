...
export default function InvoiceStagesPage() {
    ...
    const loadStages = async () => {
      try {
        ...
        if (initial) {
          ...
          try {
            const newStage = await invoiceStagesApi.createInvoiceStage({
              name: "Initial Stage",
              color: colorMap["bg-blue-500"],
              order: 1,
              type: 'initial'
    ...
    if (overId === 'success-area' || overId === 'failed-area') {
      ...
      if (stageToMove) {
        ...
        try {
          ...
          await invoiceStagesApi.updateInvoiceStage(activeIdNum, { 
            type: newType,
            order: newOrder
          });
      ...
      if (oldIndex !== -1 && newIndex !== -1) {
        ...
        try {
          await invoiceStagesApi.reorderInvoiceStages(
  ...
  const handleInitialStageColorChange = async (color: string) => {
    ...
    try {
      await invoiceStagesApi.updateInvoiceStage(initialStage.id, { color });
      setInitialStage({ ...initialStage, color });
    } catch (error: any) {
  ...
  const handleInitialStageNameChange = async (name: string) => {
    if (!initialStage) return;
    try {
      await invoiceStagesApi.updateInvoiceStage(initialStage.id, { name });
      setInitialStage({ ...initialStage, name });
      toast.success('Stage name updated');
    } catch (error: any) {
      console.error('Failed to update stage name:', error);
  ...
  const handleSave = async () => {
    try {
      ...
      await invoiceStagesApi.reorderInvoiceStages(reorderData);
