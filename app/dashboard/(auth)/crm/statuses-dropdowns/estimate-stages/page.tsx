...
export default function EstimateStagesPage() {
    ...
    const loadStages = async () => {
      try {
        ...
        if (initial) {
          ...
          try {
            const newStage = await estimateStagesApi.createEstimateStage({
              name: "Initial Stage",
              color: colorMap["bg-blue-500"],
              order: 1,
              type: 'initial'
    ...
    if (overId === 'success-area' || overId === 'failed-area') {
      ...
      if (stageToMove) {
        ...
        try {
          ...
          await estimateStagesApi.updateEstimateStage(activeIdNum, { 
            type: newType,
            order: newOrder
          });
      ...
      if (oldIndex !== -1 && newIndex !== -1) {
        ...
        try {
          await estimateStagesApi.reorderEstimateStages(
  ...
  const handleInitialStageColorChange = async (color: string) => {
    ...
    try {
      await estimateStagesApi.updateEstimateStage(initialStage.id, { color });
      setInitialStage({ ...initialStage, color });
    } catch (error: any) {
  ...
  const handleInitialStageNameChange = async (name: string) => {
    if (!initialStage) return;
    try {
      await estimateStagesApi.updateEstimateStage(initialStage.id, { name });
      setInitialStage({ ...initialStage, name });
      toast.success('Stage name updated');
    } catch (error: any) {
      console.error('Failed to update stage name:', error);
  ...
  const handleSave = async () => {
    try {
      ...
      await estimateStagesApi.reorderEstimateStages(reorderData);
